...
(() => {
  ...
  // src/web/app.ts
  var import_htmx3 = __toESM(require_htmx_min());
  ...
  // src/web/extract-json.ts
  var import_htmx2 = __toESM(require_htmx_min());
  var OnJsonEvent = class extends Event {
    constructor(json) {
      super("htmx:onJson");
      this.json = json;
    }
  };
  function getObjectField(path, json) {
    const parts = path.split(".").map((p2) => p2.trim());
    return parts.reduce((prev, curr) => prev[curr], json);
  }
  function swapJsonFields(target, json, textOnly = true) {
    target.querySelectorAll("[hx-json]").forEach((el) => {
  ...
  import_htmx2.default.defineExtension("extract-json", {
    onEvent(name, event) {
  ...
  // src/web/app.ts
  window.htmx = import_htmx3.default;
})();
